...
export class SuggestionRepository {
    static getSuggestionsByKeyword(keyword) {
        const rows = db.prepare('SELECT * FROM suggestions WHERE trigger_keyword = ?').all(keyword);
        return rows.map(row => new Suggestion(row.id, row.trigger_keyword, row.mood_id, row.text));
    }

    static getSuggestionsByMood(mood_id) {
